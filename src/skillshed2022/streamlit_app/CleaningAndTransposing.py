...
data_dict = {splitext(key)[0].split("_")[1]: pd.read_csv(
    f"{data_path}/{key}") for key in list_of_files}

# Select DataFrames for Transposing
transpose_dict = {k: v for k,
                  v in data_dict.items() if k in list(data_dict.keys())}
...
def clean_filter(df, scale_name_category):
    ...
    filtered_cols = ['SOC_Code', 'Title', 'Element_ID',
                     'Element_Name', 'Scale_Name', 'Data_Value']
