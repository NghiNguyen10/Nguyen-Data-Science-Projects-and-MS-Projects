...
dataPath = getcwd() + "/data/raw"
# print(dataPath)

df = pd.read_csv(f"{dataPath}/Iris.csv")     # df = pd.read_csv(dataPath + "/Iris.csv")
...
print('The column names', columns)
...
# The accuracy calculation is for iris data only
...
# Below are three different initializations.
# When the initialization is at the origin all points converge into one cluster
# and for the other 2 cases we get the clusters as we have initialized before(3 in this code)

# Case1. Fuzzy C-Means with cluster centres at the origin
...
# Case 2. Fuzzy C-Means with with cluster centers at random locations
# within a multi-variate Gaussian distribution with zero-mean and unit-variance.
...
def fuzzyCMeansClustering(): #Second Iteration Multivariate Gaussian
    ...
    # Case 3. Fuzzy C-Means with cluster centers at random vectors chosen from the data.
...
# Second Plot
petal_df2 = df.iloc[:,2:4]
petal_df2 = np.array(petal_df2)
...
#third initialization
m1 = random.choice(petal_df2)
m2 = random.choice(petal_df2)
m3 = random.choice(petal_df2)
cov1 = np.cov(np.transpose(petal_df2))
cov2 = np.cov(np.transpose(petal_df2))
cov3 = np.cov(np.transpose(petal_df2))
...
plt.figure(figsize=(10,10))
# creating the figure and assigning the size
plt.scatter(petal_df2[:,0], petal_df2[:,1], marker='o')     
plt.contour(X, Y, Z1.pdf(pos), colors="r" ,alpha = 0.5) 
plt.contour(X, Y, Z2.pdf(pos), colors="b" ,alpha = 0.5) 
plt.contour(X, Y, Z3.pdf(pos), colors="g" ,alpha = 0.5) 
plt.axis('equal')
...
plt.title('Initial Random Clusters(Petal) 2', fontsize=22)
# Title of the plot
plt.grid()
# displaying gridlines
plt.show()
